...
const portforlioContentDisplay=(portfolio)=>{
    ...
    var thumbnailImg=thumbnail.map((thumb,idx)=>`<li key=${idx} class='${thumb.language}'><img src="./img/port${thumb.id}.png"><div><p>${thumb.title}</p><div><button onClick="detailPage(${thumb.id})">Detail</button><a href="${thumb.site}" target="blank"><button>Site</button></a></div></div></li>`).join(" ");
...
const skillContentDisplay=(skill)=>{
    ...
    var study=study.map((study,idx)=>`<li key=${idx} ><img src="./img/${study.name}.png"/><div><span class='study' style='width:${study.percent}%;'>${study.name}</span></div></li>`).join(" ");
    var good=good.map((good,idx)=>`<li  key=${idx} ><img src="./img/${good.name}.png"/><div><span class='good' style='width:${good.percent}%' >${good.name}</span></div></li>`).join(" ");
    var greate=greate.map((great,idx)=>`<li  key=${idx} ><img src="./img/${great.name}.png"/><div><span style='width:${great.percent}%' class='great'>${great.name}</span></div></li>`).join(" ");
    
    
    var skills_img=skills.map((skill,idx)=>`<li><img src="./img/${skill}.png"/></li>`).join(" ");
    var skill_box=document.getElementById("skill_box");
    skill_type_items.innerHTML=skills_img;
    // <ul>
    //     ${skills}
    // </ul>
    // var studyImg=study.map((study,idx)=>`<li><img src="./img/${study}.png"/></li>`).join(" ");
    skill_box.innerHTML=`
 ...
 function dataLoad() { // 반환값을 받을 Promise 객체 생성
        return new Promise(function (receive) { fetch("./json/data.json") 
...
const detailPage=(idVal)=>{
    ...
    let skill_map=skill.map(skill=>`<li><img src='./img/${skill}.png'/><p>${skill}</p></li>`).join(" ");
            detail_page.innerHTML=`
                ...
                <div>
                    <div class="monitor_box" style='background-image:url("./img/port${id}.png")'>
                        <div>
  ...
  const portfolio_tap_menu=(tab)=>{
    ...
    let tabmenu=thumbnail.filter(thumb=>thumb.language==tab).map((thumb,idx)=>`<li key=${idx} class='${thumb.language}'><img src="./img/port${thumb.id}.png"><div><p>${thumb.title}</p><div><button onClick="detailPage(${thumb.id})">Detail</button><a href="${thumb.site}" target="blank"><button>Site</button></a></div></div></li>`).join(" ");
    let tabmenu_all=thumbnail.map((thumb,idx)=>`<li key=${idx} class='${thumb.language}'><img src="./img/port${thumb.id}.png"><div><p>${thumb.title}</p><div><button onClick="detailPage(${thumb.id})">Detail</button><a href="${thumb.site}" target="blank"><button>Site</button></a></div></div></li>`).join(" ");
